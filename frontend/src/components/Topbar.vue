    ...
    <div class="container">
      ...
      <ul class="nav navbar-nav pull-xs-right">
        ...
        <template v-if="isLoggedIn">
            <li class="nav-item">
              <router-link class="nav-link" :to="{name: 'auto-catalog'}" active-class="active">
                <i class="ion-compose"></i> &nbsp; Твои машины
              </router-link>
            </li>
            <li class="nav-item">
              <router-link class="nav-link" :to="{name: 'userProfile', params: {slug: currentUser.username}}" active-class="active">
                <img class="user-pic" :src="currentUser.image"> &nbsp; {{ currentUser.name }}
              </router-link>
            </li> 
        </template>
